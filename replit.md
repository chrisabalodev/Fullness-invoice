...
- `license.ts` — scrypt+timingSafeEqual password hashing; `getOrCreateLicense()` (singleton row, creates 30-day trial); public `GET /license/status` + `POST /license/redeem` (atomic, row-locked); admin (`x-admin-password` header) `verify`/`keys` list+generate/`password`; exports `licenseGuard` middleware mounted after license router, before business routers
